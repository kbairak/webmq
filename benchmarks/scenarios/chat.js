...
class ChatBenchmark {
  ...
  async setup() {
    console.log(`🚀 Setting up chat benchmark with ${this.options.userCount} users...`);
    ...
    console.log(`📡 ${this.clients.length} users connected and listening to ${this.options.chatRoom}`);
  }
  ...
  async startMessageSending() {
    console.log(`💬 Starting message sending (${this.options.messageInterval}ms interval per user)...`);
    ...
    this.clients.forEach((client, userId) => {
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          console.error(`Failed to send message from user ${userId}:`, error.message);
        }
      }, this.options.messageInterval + (Math.random() * 1000)); // Add some jitter
  ...
  startMetricsCollection() {
    ...
    this.snapshotInterval = setInterval(() => {
      ...
      console.log(`📈 ${snapshot.elapsed.toFixed(1)}s - ${snapshot.messagesPerSecond.toFixed(1)} msg/s - ${snapshot.activeConnections} connections`);
    }, 5000);
  }
  ...
  async run() {
    ...
    try {
      ...
      console.log(`⏱️  Running benchmark for ${this.options.duration / 1000} seconds...\n`);
  ...
  displayResults(report) {
    ...
    console.log(`  Total messages received: ${report.throughput.totalReceived}`);
    console.log(`  Messages per second: ${report.throughput.messagesPerSecond.toFixed(2)}`);
