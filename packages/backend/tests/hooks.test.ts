...
describe('Hook System', () => {
  describe('runWithHooks', () => {
    it('should execute function when no hooks provided', async () => {
      ...
      await runWithHooks(context, [async (ctx, next, dt) => { mockFunc(); }], data);
    ...
    it('should execute hooks in order before function', async () => {
      ...
      const hook1: HookFunction = async (context, next, data) => {
        executionOrder.push('hook1');
        await next();
      };

      const hook2: HookFunction = async (context, next, data) => {
        executionOrder.push('hook2');
        await next();
      };

      await runWithHooks({}, [hook1, hook2, async (ctx, next, dt) => { await mockFunc(); }], {});
    ...
    it('should pass context and data to each hook', async () => {
      ...
      const hook1: HookFunction = async (ctx, next, dt) => {
        hookCalls.push({ context: ctx, data: dt });
        await next();
      };

      const hook2: HookFunction = async (ctx, next, dt) => {
        hookCalls.push({ context: ctx, data: dt });
        await next();
      };

      await runWithHooks(context, [hook1, hook2], data);
    ...
    it('should allow hooks to modify context', async () => {
      ...
      const hook1: HookFunction = async (ctx, next, data) => {
        ctx.counter += 1;
        await next();
      };

      const hook2: HookFunction = async (ctx, next, data) => {
        ctx.counter += 10;
        await next();
      };

      const mockFunc = jest.fn().mockResolvedValue(undefined);
      await runWithHooks(context, [hook1, hook2, async (ctx, next, dt) => { await mockFunc(); }], {});
    ...
    it('should propagate errors from the main function', async () => {
      ...
      await expect(
        runWithHooks({}, [hook, async (ctx, next, dt) => { await mockFunc(); }], {})
    ...
    it('should work with typed data', async () => {
      ...
      const hook: HookFunction = async (context, next, dt: TestData) => {
        receivedData.push(dt);
        await next();
      };

      await runWithHooks({}, [hook], data);
